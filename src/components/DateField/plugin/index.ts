// dayjs の customParseFormat を clone してカスタムしてる。Thank you dayjs.
...
export function makeParser(format) {
  // array に ['YYYY', 'MM', 'DD'] のような形で format が格納される
  const array = format.match(formattingTokens);
  const { length } = array;
  for (let i = 0; i < length; i += 1) {
    const token = array[i];
    const parseTo = expressions[token]; // マッチする対象
    const regex = parseTo && parseTo[0]; // マッチした正規表現
    const parser = parseTo && parseTo[1]; // マッチした正規表現にマッチした時の処理
    if (parser) {
      // dayjs が認識できるフォーマットにマッチしたとき
      array[i] = { regex, parser };
    } else {
      // dayjs が認識できるフォーマットにマッチしなかったとき
  ...
  return function (input) {
    ...
    for (let i = 0, start = 0; i < length; i += 1) {
      ...
      if (typeof token === "string") {
        ...
        // フォーマットにマッチしたとき、マッチした部分を前から処理する
...
const parseFormattedInput = (input, format, utc) => {
  try {
    ...
    if (zone) {
      return new Date(
        Date.UTC(y, M, day, h, m, s, ms + zone.offset * 60 * 1000),
      );
    }
    if (utc) {
      return new Date(Date.UTC(y, M, day, h, m, s, ms));
    }
...
export default (o, C, d) => {
  ...
  proto.parse = function (cfg) {
    ...
    const format = args[1]; // format prop の中身がくる
